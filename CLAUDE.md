...
- **Prices are set by demand alone** (`valoresEnJornada` in `lib/fantasy.ts`). Everyone starts at `PRECIO_DE_SALIDA` (25 M€, deliberately below `presupuesto / tamanoEquipo` so there is slack to afford a riser). From then on a player's price is a function of **what share of the group fielded him in the last closed jornada**, not a running total: a runaway price would put a player beyond everyone's budget and, with nobody able to pick him, strand him there. The range works out at roughly 20–70 M€, which keeps the most expensive player affordable alongside four others. Goalkeepers are compared only against the other goalkeepers, since everyone must field exactly one.
- The auction price survives as `subasta` on `JugadorMercado`, as a reference only — it is not what anything costs. Nothing is stored: prices are recomputed from the saved lineups, so `getMercadoDeJornada(n)` is the only way to price a player; never read `valor` from `mercado()` for anything a user sees.
