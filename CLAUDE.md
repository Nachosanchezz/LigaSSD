...
- Prices start at the auction ones from `data/split3/equipos.ts`. The 13 who never went to auction (5 presidents + Titans) get a `TASACIONES` value in `data/fantasy.ts`.
- **Prices then move with demand** (`valoresEnJornada` in `lib/fantasy.ts`): when a jornada closes, each player gains 1 M€ per participant who picked him above the average and loses 1 M€ per participant below it. Goalkeepers are averaged only against the other goalkeepers, since everyone must field exactly one. Nothing is stored: prices are recomputed from the saved lineups of closed jornadas, so `getMercadoDeJornada(n)` is the only way to price a player — never read `valor` from `mercado()` for anything a user sees.
